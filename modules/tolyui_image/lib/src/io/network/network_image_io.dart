...
import 'package:tolyui_image/src/plugin/tolyui_image_plugin.dart';
...
class ExtendedNetworkImageProvider
    extends ImageProvider<image_provider.TolyNetworkImageProvider>
    with ExtendedImageProvider<image_provider.TolyNetworkImageProvider>
    implements image_provider.TolyNetworkImageProvider {
  ...
  ImageStreamCompleter loadImage(
    image_provider.TolyNetworkImageProvider key,
    ...
    return MultiFrameImageStreamCompleter(
      ...
      informationCollector: () {
        return <DiagnosticsNode>[
          ...
          DiagnosticsProperty<image_provider.TolyNetworkImageProvider>(
              'Image key', key),
        ];
      },
    );
  ...
  Future<Uint8List?> _loadCache(
    ...
    final Directory dir = TolyImagePlugin().imageSaveDir;
    Uint8List? data;
    // exist, try to find cache image file
    if (dir.existsSync()) {
      final File cacheFlie = File(join(dir.path, md5Key));
    ...
    if (data == null) {
      ...
      if (data != null) {
        // cache image file
        await File(join(dir.path, md5Key)).writeAsBytes(data);
      }
    }
