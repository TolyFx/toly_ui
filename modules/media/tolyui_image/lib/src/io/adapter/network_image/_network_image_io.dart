...
class FxNetworkImageProviderImpl
  ...
  ImageStreamCompleter loadImage(
    ...
    return MultiFrameImageStreamCompleter(
      codec: _loadAsync(
        key as FxNetworkImageProviderImpl,
        chunkEvents,
        decode,
      ),
      scale: key.scale,
      ...
      informationCollector: () {
        return <DiagnosticsNode>[
          ...
          DiagnosticsProperty<FxNetworkImageProvider>(
              'Image key', key),
        ];
      },
    );
  ...
  Future<ui.Codec> _loadAsync(
    FxNetworkImageProviderImpl key,
  ...
  Future<Uint8List?> _loadCache(
    ...
    final Directory dir = FxImagePlugin().imageSaveDir;
    Uint8List? data;
    // exist, try to find cache image file
    if (dir.existsSync()) {
      final File cacheFile = File(join(dir.path, md5Key));
