...
class SliverRefresh extends SingleChildRenderObjectWidget {
  ...
  void updateRenderObject(
    ...
    final RefreshStatus mode =
        TolyRefresh.of(context)!.controller.headerMode!.value;
    renderObject
...
class RenderSliverRefresh extends RenderSliverSingleBoxAdapter {
  ...
  void performLayout() {
    ...
    // overlap: 越界滑动距离 - 负数
    // layoutExtent: 刷新中，占据的滑动方向尺寸(滑动过程中为 0)
    // active: 越界滑动 或 刷新激活态  (达到滑动尺寸，且手指释放)
    ...
    final double boxExtent =
        scrollVertical ? child!.size.height : child!.size.width;
    ...
    if (active) {
      ...
      // print("===needPaintExtent:${needPaintExtent}======${refreshStyle}===${boxExtent}===${layoutExtent}=====${overscrolledExtent}=========");

      // print("==========overscrolledExtent:$overscrolledExtent===boxExtent:${boxExtent}==layoutExtent:$layoutExtent====needPaintExtent:${needPaintExtent}=========");
...
class RenderSliverLoading extends RenderSliverSingleBoxAdapter {
  ...
  double? computePaintOrigin(double? layoutExtent, bool reverse, bool follow) {
    if (follow) {
      ...
      final double value = constraints.viewportMainAxisExtent - constraints.precedingScrollExtent;
      if (reverse) {
        return Math.max(value,0.0) + layoutExtent!;
      } else {
