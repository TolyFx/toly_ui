/// ===================================================
/// Power By 张风捷特烈 --- Generated file. Do not edit.
/// github: https://github.com/toly1994328
/// ===================================================

import 'package:flutter/material.dart';
import '../../widgets.dart';

Widget widgetDisplayMap(String key) {
  return switch (key) {
    "ColorDemo1" => const ColorDemo1(),
    "ColorDemo2" => const ColorDemo2(),
    "ColorDemo3" => const ColorDemo3(),
    "ColorDemo4" => const ColorDemo4(),
    "ColorPickerDemo1" => const ColorPickerDemo1(),
    "ColorPickerDemo2" => const ColorPickerDemo2(),
    "ColorPickerDemo3" => const ColorPickerDemo3(),
    "ColorPickerDemo4" => const ColorPickerDemo4(),
    "ColorPickerDemo5" => const ColorPickerDemo5(),
    "ColorPickerDemo6" => const ColorPickerDemo6(),
    "ColorPickerDemo7" => const ColorPickerDemo7(),
    "ColorPickerDemo8" => const ColorPickerDemo8(),
    "ColorPickerDemo9" => const ColorPickerDemo9(),
    "PhoneFrameDemo1" => const PhoneFrameDemo1(),
    "ActionDemo1" => const ActionDemo1(),
    "ActionDemo2" => const ActionDemo2(),
    "ActionDemo3" => const ActionDemo3(),
    "ButtonDemo1" => const ButtonDemo1(),
    "ButtonDemo2" => const ButtonDemo2(),
    "ButtonDemo3" => const ButtonDemo3(),
    "ButtonDemo4" => const ButtonDemo4(),
    "IconDemo1" => const IconDemo1(),
    "LayoutDemo1" => const LayoutDemo1(),
    "LayoutDemo10" => const LayoutDemo10(),
    "LayoutDemo2" => const LayoutDemo2(),
    "LayoutDemo3" => const LayoutDemo3(),
    "LayoutDemo4" => const LayoutDemo4(),
    "LayoutDemo5" => const LayoutDemo5(),
    "LayoutDemo6" => const LayoutDemo6(),
    "LayoutDemo7" => const LayoutDemo7(),
    "LayoutDemo8" => const LayoutDemo8(),
    "LayoutDemo9" => const LayoutDemo9(),
    "LinkDemo1" => const LinkDemo1(),
    "LinkDemo2" => const LinkDemo2(),
    "LinkDemo3" => const LinkDemo3(),
    "TextDemo1" => const TextDemo1(),
    "TextDemo2" => const TextDemo2(),
    "TextDemo3" => const TextDemo3(),
    "TolyuiTextDemo1" => const TolyuiTextDemo1(),
    "TolyuiTextDemo2" => const TolyuiTextDemo2(),
    "TolyuiTextDemo3" => const TolyuiTextDemo3(),
    "AvatarDemo1" => const AvatarDemo1(),
    "AvatarDemo2" => const AvatarDemo2(),
    "BadgeDemo1" => const BadgeDemo1(),
    "BadgeDemo2" => const BadgeDemo2(),
    "BadgeDemo3" => const BadgeDemo3(),
    "CardDemo1" => const CardDemo1(),
    "CardDemo2" => const CardDemo2(),
    "CardDemo3" => const CardDemo3(),
    "CarouselDemo1" => const CarouselDemo1(),
    "CarouselDemo2" => const CarouselDemo2(),
    "CarouselDemo3" => const CarouselDemo3(),
    "CarouselDemo4" => const CarouselDemo4(),
    "CarouselDemo5" => const CarouselDemo5(),
    "CarouselDemo6" => const CarouselDemo6(),
    "CarouselDemo7" => const CarouselDemo7(),
    "CollapseDemo1" => const CollapseDemo1(),
    "CollapseDemo2" => const CollapseDemo2(),
    "CollapseDemo3" => const CollapseDemo3(),
    "DefaultDemo1" => const DefaultDemo1(),
    "DefaultDemo2" => const DefaultDemo2(),
    "DefaultDemo3" => const DefaultDemo3(),
    "DefaultDemo4" => const DefaultDemo4(),
    "DefaultDemo5" => const DefaultDemo5(),
    "DefaultDemo6" => const DefaultDemo6(),
    "ImageDemo1" => const ImageDemo1(),
    "FitImage" => const FitImage(),
    "AlignmentImage" => const AlignmentImage(),
    "BlendModeImage" => const BlendModeImage(),
    "RepeatImage" => const RepeatImage(),
    "CenterSliceImage" => const CenterSliceImage(),
    "PaginationDemo1" => const PaginationDemo1(),
    "PaginationDemo2" => const PaginationDemo2(),
    "ProgressDemo1" => const ProgressDemo1(),
    "ProgressDemo2" => const ProgressDemo2(),
    "ProgressDemo3" => const ProgressDemo3(),
    "ProgressDemo4" => const ProgressDemo4(),
    "ProgressDemo5" => const ProgressDemo5(),
    "SegmentedDemo1" => const SegmentedDemo1(),
    "SegmentedDemo2" => const SegmentedDemo2(),
    "SkeletonDemo1" => const SkeletonDemo1(),
    "SkeletonDemo2" => const SkeletonDemo2(),
    "SkeletonDemo3" => const SkeletonDemo3(),
    "SkeletonDemo4" => const SkeletonDemo4(),
    "SkeletonDemo5" => const SkeletonDemo5(),
    "StatisticsDemo" => const StatisticsDemo(),
    "CountdownDemo" => const CountdownDemo(),
    "StatisticsDemo3" => const StatisticsDemo3(),
    "TableDemo1" => const TableDemo1(),
    "TableDemo2" => const TableDemo2(),
    "TableDemo3" => const TableDemo3(),
    "TableDemo4" => const TableDemo4(),
    "TableDemo5" => const TableDemo5(),
    "TableDemo7" => const TableDemo7(),
    "TableDemo10" => const TableDemo10(),
    "TableDemo12" => const TableDemo12(),
    "TagDemo1" => const TagDemo1(),
    "TagDemo2" => const TagDemo2(),
    "TagDemo3" => const TagDemo3(),
    "TagDemo4" => const TagDemo4(),
    "TagDemo5" => const TagDemo5(),
    "TimelineDemo1" => const TimelineDemo1(),
    "TimelineDemo2" => const TimelineDemo2(),
    "TimelineDemo3" => const TimelineDemo3(),
    "TimelineDemo4" => const TimelineDemo4(),
    "TimelineDemo5" => const TimelineDemo5(),
    "TimelineDemo6" => const TimelineDemo6(),
    "TreeDemo1" => const TreeDemo1(),
    "TreeDemo2" => const TreeDemo2(),
    "TreeDemo3" => const TreeDemo3(),
    "TreeDemo4" => const TreeDemo4(),
    "TreeDemo5" => const TreeDemo5(),
    "TreeDemo6" => const TreeDemo6(),
    "TreeDemo7" => const TreeDemo7(),
    "WatermarkDemo1" => const WatermarkDemo1(),
    "WatermarkDemo2" => const WatermarkDemo2(),
    "WatermarkDemo3" => const WatermarkDemo3(),
    "WatermarkDemo4" => const WatermarkDemo4(),
    "WatermarkDemo5" => const WatermarkDemo5(),
    "LoadingDemo1" => const LoadingDemo1(),
    "LoadingDemo2" => const LoadingDemo2(),
    "LoadingDemo3" => const LoadingDemo3(),
    "MessageDemo1" => const MessageDemo1(),
    "MessageDemo2" => const MessageDemo2(),
    "MessageDemo3" => const MessageDemo3(),
    "MessageDemo4" => const MessageDemo4(),
    "MessageDemo5" => const MessageDemo5(),
    "NotificationDemo1" => const NotificationDemo1(),
    "NotificationDemo2" => const NotificationDemo2(),
    "NotificationDemo3" => const NotificationDemo3(),
    "PopoverDemo1" => const PopoverDemo1(),
    "PopoverDemo2" => const PopoverDemo2(),
    "PopoverDemo3" => const PopoverDemo3(),
    "PopoverDemo4" => const PopoverDemo4(),
    "CallbackShortcutsExample" => const CallbackShortcutsExample(),
    "CharacterActivatorExample" => const CharacterActivatorExample(),
    "TooltipDemo1" => const TooltipDemo1(),
    "TooltipDemo2" => const TooltipDemo2(),
    "TooltipDemo3" => const TooltipDemo3(),
    "AutocompleteDemo1" => const AutocompleteDemo1(),
    "AutocompleteDemo2" => const AutocompleteDemo2(),
    "CheckboxDemo1" => const CheckboxDemo1(),
    "CheckboxDemo2" => const CheckboxDemo2(),
    "CheckboxDemo3" => const CheckboxDemo3(),
    "CheckboxDemo4" => const CheckboxDemo4(),
    "DatePickerDemo1" => const DatePickerDemo1(),
    "DatePickerDemo2" => const DatePickerDemo2(),
    "DatePickerDemo3" => const DatePickerDemo3(),
    "InputDemo1" => const InputDemo1(),
    "InputDemo2" => const InputDemo2(),
    "InputDemo3" => const InputDemo3(),
    "InputDemo4" => const InputDemo4(),
    "InputDemo5" => const InputDemo5(),
    "RadioDemo1" => const RadioDemo1(),
    "RadioDemo2" => const RadioDemo2(),
    "RadioDemo3" => const RadioDemo3(),
    "RadioDemo4" => const RadioDemo4(),
    "SelectDemo1" => const SelectDemo1(),
    "SelectDemo2" => const SelectDemo2(),
    "SliderDemo1" => const SliderDemo1(),
    "SliderDemo2" => const SliderDemo2(),
    "SliderDemo3" => const SliderDemo3(),
    "SliderDemo4" => const SliderDemo4(),
    "SwitchDemo1" => const SwitchDemo1(),
    "SwitchDemo2" => const SwitchDemo2(),
    "SwitchDemo3" => const SwitchDemo3(),
    "SwitchDemo4" => const SwitchDemo4(),
    "TransferDemo1" => const TransferDemo1(),
    "TransferDemo2" => const TransferDemo2(),
    "AnchorDemo1" => const AnchorDemo1(),
    "AnchorDemo2" => const AnchorDemo2(),
    "AnchorDemo4" => const AnchorDemo4(),
    "AnchorDemo3" => const AnchorDemo3(),
    "BreadcrumbDemo1" => const BreadcrumbDemo1(),
    "BreadcrumbDemo2" => const BreadcrumbDemo2(),
    "BreadcrumbDemo3" => const BreadcrumbDemo3(),
    "BreadcrumbDemo4" => const BreadcrumbDemo4(),
    "DropMenuDemo1" => const DropMenuDemo1(),
    "DropMenuDemo2" => const DropMenuDemo2(),
    "DropMenuDemo3" => const DropMenuDemo3(),
    "DropMenuDemo4" => const DropMenuDemo4(),
    "DropMenuDemo5" => const DropMenuDemo5(),
    "DropMenuDemo6" => const DropMenuDemo6(),
    "DropMenuDemo7" => const DropMenuDemo7(),
    "RailMenuBarDemo1" => const RailMenuBarDemo1(),
    "RailMenuBarDemo2" => const RailMenuBarDemo2(),
    "RailMenuBarDemo3" => const RailMenuBarDemo3(),
    "RailMenuTreeDemo1" => const RailMenuTreeDemo1(),
    "RailMenuTreeDemo2" => const RailMenuTreeDemo2(),
    "RailMenuTreeDemo3" => const RailMenuTreeDemo3(),
    "RailMenuTreeDemo4" => const RailMenuTreeDemo4(),
    "StepsDemo1" => const StepsDemo1(),
    "StepsDemo2" => const StepsDemo2(),
    "StepsDemo3" => const StepsDemo3(),
    "StepsDemo4" => const StepsDemo4(),
    "StepsDemo5" => const StepsDemo5(),
    "TabsDemo1" => const TabsDemo1(),
    "TabsDemo2" => const TabsDemo2(),
    "TabsDemo3" => const TabsDemo3(),
    "TabsDemo4" => const TabsDemo4(),
    "TabsDemo5" => const TabsDemo5(),
    "TabsDemo6" => const TabsDemo6(),
    _ => const SizedBox(),
  };
}
